...
////=============================================            
    ...
    public void aggregate() {
    	// connect to sqlite
    	String table_sql = "DROP TABLE POSs; CREATE TABLE POSs (IDent integer primary key autoincrement, Key TEXT, Entity TEXT, Count integer);"; //NER
		c = sqllite.init(table_sql);
		
    	ArrayList<String> keys = new ArrayList<String>( Arrays.asList("NN", "NNS", "NP", "NPS") );// NN NNS NP NPS "LOCATION", "ORGANIZATION", "PERSON"
    	for (String key : keys) {
	    	DBObject unwind = new BasicDBObject("$unwind", "$clustering_pos." + key); //ner
	    	
	    	//the $group operation
	    	DBObject groupFields = new BasicDBObject( "_id", "$clustering_pos." + key);
	    	...
	    	for (DBObject result : output.results()) {
	    		...
	    		try {
	    			// sql INSERT query
					PreparedStatement ps = c.prepareStatement("INSERT INTO POSs (Key, Entity, Count) VALUES (?,?,?)"); //NER
					ps.setString(1, key);
